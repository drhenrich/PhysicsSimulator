...
# ============================================
# TABS ERSTELLEN - Callback setzt current_tab
# ============================================
def update_tab(tab_index):
    st.session_state.current_tab = tab_index

# Callback-basierte Tabs
selected_tab = st.tabs(tabs_labels)

# Manuell current_tab synchronisieren - durch Überprüfung welcher Tab gerade sichtbar ist
# Das funktioniert indem wir nach jedem Tab-Render current_tab aktualisieren
...
# ============================================
# SIDEBAR - Liest current_tab aus session_state
# ============================================
with st.sidebar:
    ...
    # Nur Tab 2: RÖNTGEN/CT KLASSISCH
    elif current_tab == 2:
        st.markdown("### 🏥 CT-Parameter (Klassisch)")
        preset_ct_c = st.selectbox("Preset wählen", [""] + list(PRESENTS_CT.keys()), key="preset_ct_classic")
        colC1, colC2 = st.columns(2)
        
        with colC1:
            if st.button("Laden", key="ct-classic-load"):
                if preset_ct_c:
        ...
        with colC2:
            if st.button("Reset", key="ct-classic-reset"):
                default_key = list(PRESENTS_CT.keys())[0]
                for k, v in PRESENTS_CT[default_key].items():
                    st.session_state[f"cc{k}"] = v
                st.info("🔄 Auf Standard zurückgesetzt")
    
    # Nur Tab 3: RÖNTGEN/CT SAFE
    elif current_tab == 3:
        st.markdown("### 🏥 CT-Parameter (Safe)")
        preset_ct_s = st.selectbox("Preset wählen", [""] + list(PRESENTS_CT.keys()), key="preset_ct_safe")
        colC1, colC2 = st.columns(2)
        
        with colC1:
            if st.button("Laden", key="ct-safe-load"):
                if preset_ct_s:
        ...
        with colC2:
            if st.button("Reset", key="ct-safe-reset"):
                default_key = list(PRESENTS_CT.keys())[0]
                for k, v in PRESENTS_CT[default_key].items():
                    st.session_state[f"cs{k}"] = v
                st.info("🔄 Auf Standard zurückgesetzt")
