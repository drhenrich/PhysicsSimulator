...
with tabs[2]:
    ...
    with st.sidebar:
        ...
        preset_ct = st.selectbox("Preset wählen", [""] + list(PRESENTS_CT.keys()), key="preset_ct_classic")
...
with tabs[3]:
    ...
    with st.sidebar:
        ...
        preset_ct_safe = st.selectbox("Preset wählen", [""] + list(PRESENTS_CT.keys()), key="preset_ct_safe")
        colC1, colC2 = st.columns(2)
        
        with colC1:
            if st.button("Laden", key="ct-safe-load"):
                if preset_ct_safe:
                    for k, v in PRESENTS_CT[preset_ct_safe].items():
                        st.session_state[f"cs{k}"] = v
                    st.success(f"✅ {preset_ct_safe} geladen")
...
with tabs[5]:
    ...
    # Sidebar für diesen Tab (ohne Presets)
    with st.sidebar:
        st.markdown(f"## {get_text('settings', st.session_state.language) or 'Einstellungen'}")
        st.markdown("---")
